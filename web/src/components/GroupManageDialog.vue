<template>
  ...
  >
    <div class="gm-hint">拖动 ☰ 手柄排序；点击分组查看基金；点击基金右侧分组名可移动</div>
    ...
    <ul ref="groupListRef" class="gm-list">
      ...
      >
        ...
        <el-button size="small" link type="primary" @click.stop="renameGroup(g)">
    ...
    <ul ref="fundListRef" class="gm-list">
      ...
      >
        ...
        <el-select
          ...
          @click.stop
          @change="(val: string) => moveFundToGroup(fund.code, val || '__uncategorized__')"
...
const selectedGroupLabel = computed(() => {
  ...
  return draftGroupOrder.value.find(x => x.key === selectedGroup.value)?.name || ''
})
...
async function addNewGroup() {
  try {
    ...
    if (!validateGroupName(name)) { ElMessage.warning('分组名称无效'); return }
    if (draftGroupOrder.value.some(g => g.name === name)) { ElMessage.warning('分组名称已存在'); return }
...
async function renameGroup(g: { key: string; name: string }) {
  try {
    ...
    const name = String(value).trim()
    if (!validateGroupName(name)) { ElMessage.warning('分组名称无效'); return }
    if (draftGroupOrder.value.some(x => x.name === name && x.key !== g.key)) { ElMessage.warning('分组名称已存在'); return }
...
async function deleteGroup(g: { key: string; name: string }) {
  try {
    ...
    if (selectedGroup.value === g.key) selectedGroup.value = '__uncategorized__'
  } catch { /* cancel */ }
}
...
function destroySortables() {
  groupSortable?.destroy(); groupSortable = null
  fundSortable?.destroy(); fundSortable = null
}
  ...
  fundSortable = Sortable.create(el, {
    ...
    onEnd(evt) {
      ...
      if (selectedGroup.value === '__uncategorized__') {
        ...
        if (moved) { codes.splice(newIndex, 0, moved); fundStore.reorderFunds(codes) }
      } else {
        const codes = draftFundGroups.value.get(selectedGroup.value)
        if (!codes) return
        const [moved] = codes.splice(oldIndex, 1)
  ...
  try {
    ...
    for (const g of draftGroupOrder.value) {
      if (!existingKeys.has(g.key)) {
        ...
        if (!groupStore.groupOrder.includes(g.key)) groupStore.groupOrder.push(g.key)
      }
    }
    ...
    for (const fund of fundStore.funds) {
      ...
      for (const [groupKey, codes] of draftFundGroups.value.entries()) {
        if (codes.includes(fund.code)) { foundGroup = groupKey; break }
      }
