...
export function getScripts(): string[] {
  return [
    ...
    '  /**',
    '   * 获取最新交易日日期（用于表头显示）',
    '   * 如果今天已经收盘（gztime 的日期 == netValueDate），则显示今天',
    '   * 否则显示上一个交易日的日期',
    '   */',
    ...
    '      ',
    '      if (groupStats[g] !== undefined) {',
    '        groupStats[g] += estimatedGain;',
    '      }',
    '      if (g !== "全部") {',
    '        groupStats["全部"] += estimatedGain;',
    '      }',
    '    });',
    '    ',
    ...
    '      const stat = groupStats[g] || 0;',
    '      const text = `${g}`;',
    '      const tooltipTitle = `分组收益: ${stat >= 0 ? "+" : ""}${stat.toFixed(2)}`;',
    '      return `<div class="group-tag-btn ${isActive ? "active" : ""}" data-group="${g}" title="${tooltipTitle}">${text}</div>`;',
    '    }).join("");',
    '',
    ...
    '    const estimatedGain = (gsz - dwjz) * num;',
    '',
    ...
    '    const estimatedDate = extractDate(fund.gztime); // 估算日期（当前开盘日）',
    ...
    '      estimatedDate: estimatedDate, // 估算日期（当前开盘日）',
    ...
    '        <div class="${estimatedChangeColor} td-sub">${formatPercent(gszzl)}</div>',
    '      </td>`,',
