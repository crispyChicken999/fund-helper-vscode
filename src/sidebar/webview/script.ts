...
export function getScripts(): string[] {
  return [
    ...
    '    // 判断是否未开盘（交易日但还未开盘）',
    '    const isMarketNotOpenedYet = !marketStatus.isOpen && !isMarketClosedNow;',
    '',
    '    // 估算收益（盘中）- 使用估算净值和单位净值的差值',
    '    // 如果休市或未开盘，则不显示估算收益',
    '    const estimatedGain = (isMarketClosedNow || isMarketNotOpenedYet) ? 0 : (gsz - dwjz) * num;',
    '',
    ...
    '',
    '    // 颜色类',
    '    const estimatedChangeColor = (isMarketClosedNow || isMarketNotOpenedYet) ? \'color-flat\' : getColorClass(gszzl);',
    '    const dailyChangeColor = getColorClass(navChgRt);',
    '    const estimatedGainColor = (isMarketClosedNow || isMarketNotOpenedYet) ? \'color-flat\' : getColorClass(estimatedGain);',
    ...
    '      code: fund.code,',
    '      gsz: gsz.toFixed(4),',
    '      gszzl: (isMarketClosedNow || isMarketNotOpenedYet) ? "--" : formatPercent(gszzl),',
    '      dwjz: dwjz.toFixed(4),',
    '      navChgRt: formatPercent(navChgRt),',
    '      estimatedGain: (isMarketClosedNow || isMarketNotOpenedYet) ? "--" : formatMoney(estimatedGain),',
    ...
    '        <div class="${estimatedChangeColor}">${(isMarketClosedNow || isMarketNotOpenedYet) ? "--" : formatPercent(gszzl)}</div>',
    '        <div class="td-sub">${isMarketClosedNow ? "休市" : (isMarketNotOpenedYet ? estimatedDate : updateTime)}</div>',
    '      </td>`,',
    '      estimatedGain: `<td class="td-value" data-column="estimatedGain">',
    '        <div class="${estimatedGainColor} privacy-hide-amount">${(isMarketClosedNow || isMarketNotOpenedYet) ? "--" : formatMoney(estimatedGain)}</div>',
    '        <div class="${estimatedChangeColor} td-sub">${(isMarketClosedNow || isMarketNotOpenedYet) ? "--" : formatPercent(gszzl)}</div>',
    '      </td>`,',
