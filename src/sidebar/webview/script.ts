...
export function getScripts(): string[] {
  return [
    ...
    '        weightedEstimatedChange: 0, // 加权估算涨幅',
    ...
    '      const estimatedGain = (holdingAmount * gszzl) / 100; // 估算收益（基于总资产）',
    ...
    '        groupStats[g].weightedEstimatedChange += holdingAmount * gszzl;',
    ...
    '        groupStats["全部"].weightedEstimatedChange += holdingAmount * gszzl;',
    ...
    '    // 只有在休市 OR API数据未更新时才显示 0',
    '    const estimatedGain = (isMarketClosedNow || !hasUpdatedData) ? 0 : (gsz - dwjz) * num;',
    '',
    ...
    '    // 估算日期显示逻辑：',
    '    // - 如果休市，显示"休市"',
    '    // - 如果 API 数据未更新到今天，显示今天的日期（暗示数据未刷新）',
    ...
    '    const fullUpdateTime = fund.gztime ? formatUpdateTime(fund.gztime) : "";',
    '',
    '    // 关联板块',
    '    const relateTheme = fund.relateTheme || \'--\';',
    '',
    '    // 颜色类',
    '    const estimatedChangeColor = (isMarketClosedNow || !hasUpdatedData) ? \'color-flat\' : getColorClass(gszzl);',
    '    const dailyChangeColor = getColorClass(navChgRt);',
    '    const estimatedGainColor = (isMarketClosedNow || !hasUpdatedData) ? \'color-flat\' : getColorClass(estimatedGain);',
    ...
    '      code: fund.code,',
    '      gsz: gsz.toFixed(4),',
    '      gszzl: (isMarketClosedNow || !hasUpdatedData) ? "--" : formatPercent(gszzl),',
    '      dwjz: dwjz.toFixed(4),',
    '      navChgRt: formatPercent(navChgRt),',
    '      estimatedGain: (isMarketClosedNow || !hasUpdatedData) ? "--" : formatMoney(estimatedGain),',
    ...
    '        <div class="${estimatedChangeColor}">${(isMarketClosedNow || !hasUpdatedData) ? "--" : formatPercent(gszzl)}</div>',
    '        <div class="td-sub">${isMarketClosedNow ? "休市" : (hasUpdatedData ? updateTime : estimatedDate)}</div>',
    '      </td>`,',
    '      estimatedGain: `<td class="td-value" data-column="estimatedGain">',
    '        <div class="${estimatedGainColor} privacy-hide-amount">${(isMarketClosedNow || !hasUpdatedData) ? "--" : formatMoney(estimatedGain)}</div>',
    '        <div class="${estimatedChangeColor} td-sub">${(isMarketClosedNow || !hasUpdatedData) ? "--" : formatPercent(gszzl)}</div>',
    '      </td>`,',
