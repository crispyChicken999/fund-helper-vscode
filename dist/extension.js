...
async function fetchSingleFund(code) {
  const url2 = `https://fundgz.1234567.com.cn/js/${code}.js?rt=${Date.now()}`;
  const res = await axios_default.get(url2, { timeout: 1e4, responseType: "text" });
  return parseJsonp(res.data);
}
async function getFundData(configs, previousData = []) {
  if (configs.length === 0) {
    return [];
  }
...
async function searchFund(keyword) {
  ...
  const url2 = `https://fundsuggest.eastmoney.com/FundSearch/api/FundSearchAPI.ashx?m=9&key=${encodeURIComponent(keyword)}&_=${Date.now()}`;
...
async function getNetValueHistory(fundCode, pageSize = 10) {
  const url2 = `https://api.fund.eastmoney.com/f10/lsjz?fundCode=${fundCode}&pageIndex=1&pageSize=${pageSize}&startDate=&endDate=&_=${Date.now()}`;
  const res = await axios_default.get(url2, {
    timeout: 1e4,
    headers: { Referer: "https://fundf10.eastmoney.com/" }
  });
  const list = res.data?.Data?.LSJZList ?? [];
...
// src/fundModel.ts
function calcHoldingAmount(fund) {
  return fund.netValue * fund.shares;
}
function calcDailyGain(fund) {
  if (fund.shares <= 0) {
    return 0;
  }
  if (fund.isRealValue) {
    return (fund.netValue - fund.netValue / (1 + fund.changePercent * 0.01)) * fund.shares;
  }
...
var FundTreeItem = class extends vscode.TreeItem {
  constructor(label, collapsibleState, fundInfo, isDetailItem = false) {
...
var FundTreeDataProvider = class {
  constructor(extensionPath) {
    ...
    this._sortMethod = vscode.workspace.getConfiguration("fund-helper").get("sortMethod", "default");
  }
  get fundDataList() {
  ...
  toggleSort(field) {
    ...
    if (currentField === field) {
      if (currentDir === "desc") {
        this._sortMethod = `${field}_asc`;
      } else {
        this._sortMethod = "default";
      }
    } else {
      this._sortMethod = `${field}_desc`;
    }
    this._saveSortMethod(this._sortMethod);
    this._onDidChangeTreeData.fire();
  }
  _saveSortMethod(method) {
    vscode.workspace.getConfiguration("fund-helper").update("sortMethod", method, vscode.ConfigurationTarget.Global);
  }
  _getFundConfigs() {
    return vscode.workspace.getConfiguration("fund-helper").get("funds", []);
  }
  _getSortedFundItems() {
  ...
  _createSortHeader() {
    ...
    const pickColor = (v) => v > 0 ? UP : v < 0 ? DOWN : NEUTRAL;
    const hl = (text, color) => `**<span style="color:${color};background-color:${color}33;">&nbsp;${text}&nbsp;</span>**`;
    const signFmt = (v) => `${v >= 0 ? "+" : ""}${v.toFixed(2)}`;
    let totalAmount = 0;
    ...
    const totalHoldingRate = totalAmount > 0 ? totalHoldingGain / (totalAmount - totalHoldingGain) * 100 : 0;
    const md = new vscode.MarkdownString();
    md.isTrusted = true;
    md.supportThemeIcons = true;
    md.supportHtml = true;
    md.appendMarkdown(`#### \u{1F4CA} \u6301\u4ED3\u6982\u89C8
...
`);
  ...
  _buildTooltip(fund) {
    ...
    const holdingGainRate = calcHoldingGainRate(fund);
    const dailyGain = calcDailyGain(fund);
    const pickColor = (val) => val > 0 ? UP : val < 0 ? DOWN : NEUTRAL;
    ...
    const holdRateStr = fund.cost > 0 ? `${sign(holdingGainRate)}${holdingGainRate.toFixed(2)}%` : "--";
    const estValueStr = fund.estimatedValue !== null ? fund.estimatedValue.toFixed(4) : "--";
    const updateStr = fund.updateTime ? fund.updateTime.length > 10 ? fund.updateTime.substring(11) : fund.updateTime : "--";
    const md = new vscode.MarkdownString();
    md.isTrusted = true;
    md.supportThemeIcons = true;
    md.supportHtml = true;
...
`);
  ...
  _getDetailItems(fund) {
    const holdingAmount = calcHoldingAmount(fund);
    const holdingGain = calcHoldingGain(fund);
    const holdingGainRate = calcHoldingGainRate(fund);
    const dailyGain = calcDailyGain(fund);
    const dot = (val) => val > 0 ? "\u{1F534}" : val < 0 ? "\u{1F7E2}" : "\u26AA";
    const signStr = (val) => `${val >= 0 ? "+" : ""}${val.toFixed(2)}`;
    const details = [
      ...
      {
        label: `\u6301\u6709\u6536\u76CA\u7387`,
        value: fund.cost > 0 ? `${dot(holdingGainRate)} ${signStr(holdingGainRate)}%` : "--"
      },
      ...
      {
        label: "\u66F4\u65B0\u65F6\u95F4",
        value: fund.updateTime ? fund.updateTime.length > 10 ? fund.updateTime.substring(11) : fund.updateTime : "--"
      }
    ];
...
var FUND_MIME = "application/vnd.fund-helper.fund";
var FundDragAndDropController = class {
  constructor(provider) {
    this.dragMimeTypes = [FUND_MIME];
    this.dropMimeTypes = [FUND_MIME];
  ...
  async handleDrop(target, dataTransfer, _token) {
    ...
    const draggedCode = dataTransfer.get(FUND_MIME)?.value;
...
function updateStatusBar(fundList) {
  if (!statusBarItem) {
    return;
  }
...
function activate(context) {
  ...
  const treeView = vscode3.window.createTreeView("fundList", {
    treeDataProvider,
  ...
  context.subscriptions.push(
    vscode3.commands.registerCommand("fund-helper.refreshFund", async () => {
      await refreshData();
      vscode3.window.showInformationMessage("\u57FA\u91D1\u6570\u636E\u5DF2\u5237\u65B0");
    }),
    vscode3.commands.registerCommand("fund-helper.addFund", () => addFund()),
    ...
    vscode3.commands.registerCommand("fund-helper.sortFund", () => sortFund()),
    vscode3.commands.registerCommand(
      "fund-helper.exportFund",
      () => exportFund()
    ),
    vscode3.commands.registerCommand(
      "fund-helper.importFund",
      () => importFund()
    ),
    // 排序快捷命令（inline 按钮用）
    vscode3.commands.registerCommand("fund-helper.sortByDefault", () => {
      treeDataProvider.sortMethod = "default";
    }),
    vscode3.commands.registerCommand("fund-helper.sortByChangePercent", () => {
      treeDataProvider.toggleSort("changePercent");
    }),
    vscode3.commands.registerCommand("fund-helper.sortByDailyGain", () => {
      treeDataProvider.toggleSort("dailyGain");
    }),
    vscode3.commands.registerCommand("fund-helper.sortByHoldingAmount", () => {
      treeDataProvider.toggleSort("holdingAmount");
    }),
    vscode3.commands.registerCommand("fund-helper.sortByHoldingGain", () => {
      treeDataProvider.toggleSort("holdingGain");
    }),
    vscode3.commands.registerCommand("fund-helper.sortByHoldingGainRate", () => {
...
function getFundConfigs() {
  return vscode3.workspace.getConfiguration("fund-helper").get("funds", []);
}
async function saveFundConfigs(funds) {
  await vscode3.workspace.getConfiguration("fund-helper").update("funds", funds, vscode3.ConfigurationTarget.Global);
}
async function addFund() {
  const keyword = await vscode3.window.showInputBox({
  ...
  try {
    ...
    if (results.length === 0) {
      vscode3.window.showWarningMessage("\u672A\u627E\u5230\u5339\u914D\u7684\u57FA\u91D1");
      return;
    }
    ...
    if (addedCount > 0) {
      ...
      await refreshData();
      vscode3.window.showInformationMessage(`\u6210\u529F\u6DFB\u52A0 ${addedCount} \u4E2A\u57FA\u91D1`);
    } else {
      vscode3.window.showWarningMessage("\u6240\u9009\u57FA\u91D1\u5DF2\u5728\u5217\u8868\u4E2D");
    }
  } catch (e) {
    vscode3.window.showErrorMessage(`\u641C\u7D22\u57FA\u91D1\u5931\u8D25: ${e.message}`);
  }
}
async function deleteFund(code) {
  ...
  await saveFundConfigs(funds);
  await refreshData();
  vscode3.window.showInformationMessage(`\u5DF2\u5220\u9664\u57FA\u91D1 ${code}`);
}
async function addPosition(code, name) {
  let historyItems;
  try {
    const history = await getNetValueHistory(code, 15);
    if (history.length === 0) {
      vscode3.window.showWarningMessage("\u672A\u83B7\u53D6\u5230\u5386\u53F2\u51C0\u503C\u6570\u636E");
      return;
    }
    ...
    vscode3.window.showErrorMessage(`\u83B7\u53D6\u5386\u53F2\u51C0\u503C\u5931\u8D25: ${e.message}`);
    return;
  }
  const selected = await vscode3.window.showQuickPick(historyItems, {
    placeHolder: `\u52A0\u4ED3 ${name} \u2014 \u8BF7\u9009\u62E9\u4E70\u5165\u65E5\u671F\u7684\u51C0\u503C`
  });
  if (!selected) {
    return;
  }
  const numStr = await vscode3.window.showInputBox({
    prompt: `\u52A0\u4ED3 ${name} \u2014 \u4E70\u5165\u51C0\u503C ${selected.netValue.toFixed(4)}\uFF0C\u8BF7\u8F93\u5165\u52A0\u4ED3\u4EFD\u989D`,
    placeHolder: "\u4F8B\u5982\uFF1A1000",
    validateInput: (v) => isNaN(Number(v)) || Number(v) <= 0 ? "\u8BF7\u8F93\u5165\u6B63\u6570" : void 0
  });
  if (!numStr) {
    return;
  }
...
async function reducePosition(code, name) {
  const funds = getFundConfigs();
  const fund = funds.find((f) => f.code === code);
  if (!fund) {
    return;
  ...
  if (currentNum <= 0) {
    vscode3.window.showWarningMessage("\u5F53\u524D\u65E0\u6301\u4ED3");
    return;
  }
  let historyItems;
  try {
  ...
  await saveFundConfigs(funds);
  await refreshData();
  vscode3.window.showInformationMessage(`\u51CF\u4ED3\u6210\u529F\uFF01\u5269\u4F59\u4EFD\u989D: ${fund.num}`);
}
async function editPosition(code, name) {
  const funds = getFundConfigs();
  const fund = funds.find((f) => f.code === code);
  if (!fund) {
    return;
  ...
  const numStr = await vscode3.window.showInputBox({
    ...
    value: fund.num,
    validateInput: (v) => isNaN(Number(v)) || Number(v) < 0 ? "\u8BF7\u8F93\u5165\u975E\u8D1F\u6570" : void 0
  });
  if (numStr === void 0) {
    return;
  }
  const costStr = await vscode3.window.showInputBox({
...
async function sortFund() {
  ...
  if (selected) {
    treeDataProvider.sortMethod = selected.method;
  }
}
async function importFund() {
  ...
  try {
    ...
    if (Array.isArray(data.funds)) {
      ...
      vscode3.window.showErrorMessage("JSON \u683C\u5F0F\u4E0D\u6B63\u786E\uFF0C\u9700\u8981\u5305\u542B funds \u6570\u7EC4");
      return;
    }
    const newFunds = fundsList.map((f) => ({
      code: String(f.code || ""),
      num: String(f.num ?? f.shares ?? "0"),
      cost: String(f.cost ?? "0")
    })).filter((f) => f.code.length > 0);
    if (newFunds.length === 0) {
      vscode3.window.showWarningMessage("\u672A\u627E\u5230\u6709\u6548\u7684\u57FA\u91D1\u6570\u636E");
      return;
    }
    ...
    vscode3.window.showErrorMessage(`\u5BFC\u5165\u5931\u8D25: ${e.message}`);
  }
}
async function exportFund() {
  ...
  const uri = await vscode3.window.showSaveDialog({
    defaultUri: vscode3.Uri.file("fund-list.json"),
    filters: { "JSON \u6587\u4EF6": ["json"] }
  });
  if (uri) {
    fs.writeFileSync(uri.fsPath, JSON.stringify(exportData, null, 2), "utf-8");
    vscode3.window.showInformationMessage(`\u57FA\u91D1\u5217\u8868\u5DF2\u5BFC\u51FA\u5230 ${uri.fsPath}`);
  }
}
function setupAutoRefresh() {
  ...
  const config = vscode3.workspace.getConfiguration("fund-helper");
...
function isDuringTradeTime() {
  const now = /* @__PURE__ */ new Date();
  const utc = now.getTime() + now.getTimezoneOffset() * 6e4;
  const cn = new Date(utc + 8 * 36e5);
  const day = cn.getDay();
  if (day === 0 || day === 6) {
    return false;
  }
